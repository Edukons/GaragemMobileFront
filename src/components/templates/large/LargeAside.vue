<script setup>
import LogoTitle from '@/components/templates/LogoTitle.vue';
</script>
<template>
    <div class="logo_and_menu">
    <logo-title class="mb-2" />
    <div class="divider" />
    <div class="menu">
      <router-link to="/">
        <i class="icon mdi mdi-home-outline" /> Home
      </router-link>
    </div>
    <div class="divider" />
    <div class="menu">
      <router-link to="/acessorios">
        <i class="icon mdi mdi-flashlight " /> Acessorio
      </router-link>
      <router-link to="/categorias">
        <i class="icon mdi mdi-notification-clear-all" /> Categoria
      </router-link>
      <router-link to="/cores">
        <i class="icon mdi mdi-opacity" /> Cor
      </router-link>
      <router-link to="/marcas">
        <i class="icon mdi mdi-numeric-0-box-multiple-outline" /> Marca
      </router-link>
      <router-link to="/login">
        <i class="icon mdi mdi-account-plus" /> Login
      </router-link>
    </div>
  </div>
  <logo-title />
</template>
<style scoped>
.mb-2 {
  margin-bottom: 1.5rem;
}

.icon {
  font-size: 2rem;
  margin-left: 15px;
}

.divider {
  margin-top: 1rem;
  border-top: 1px solid #355783;
}

.menu {
  padding: 3rem;
  background-color: #355783;
}

.menu a {
  display: flex;
  align-items: center;
  text-decoration: none;
  color: #000000;
  gap: 1rem;
  font-size: 1.3rem;
  margin-top: 2.2rem;
  background-color: #F29F05;
  border-radius: 30px;
}
</style>